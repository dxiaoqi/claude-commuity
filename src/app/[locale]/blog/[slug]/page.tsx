...
export function generateStaticParams() {
  const slugs = getSlugsByKind("blog");
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) return {};
  if (post.kind === "lab") {
    return { alternates: { canonical: `/${locale}/lab/${slug}` } };
  }
  ...
  const languagesEntries: [string, string][] = locales
    ...
    .map((l) => [l, `/${l}/blog/${slug}`]);
  languagesEntries.push(["x-default", `/${defaultLocale}/blog/${slug}`]);
  return {
    title: post.title,
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const post = getPost(slug, locale);
  if (!post) notFound();
  if (post.kind === "lab") redirect(`/${locale}/lab/${slug}`);
  ...
  return (
    <main id="main" className="post-shell shell-width">
      ...
      <div className="post-breadcrumb">
        <Link href={`/${locale}/blog`}>← {dict.blog.allArticles}</Link>
      </div>
      ...
      <footer className="post-footer">
        <Link className="back-link" href={`/${locale}/blog`}>← {dict.blog.backToAll}</Link>
      </footer>
    </main>
  );
}
