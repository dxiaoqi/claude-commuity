...
We now have an agent that reads files, runs commands, and edits code. It also has a problem: after 15–20 turns in a real coding session, its `history` is 50 KB of mostly-stale tool output. The model is paying to re-attend to that noise on every request, and — per the [context rot](/en/blog/context-engineering-for-coding-agents) research — it's getting less accurate as the window fills up.
